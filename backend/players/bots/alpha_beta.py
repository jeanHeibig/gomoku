...
def sort_moves(move_scores):
    ...
    for k in range(64):
        score = move_scores[k]
        if score >= 0:  # score = -1 for occupied cells
...
def find_best_move(bb_current, bb_opponent, max_depth, time_limit):
    ...
    # for depth in range(1, max_depth + 1):  #TODO: put it back
...
def ab_bot(position, current_player, timer, _):
    ...
    move_time = 2 * remaining_time / (len(moves) + 1)
    move_time = 15  # TODO: remove
